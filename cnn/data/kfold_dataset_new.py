...
class KFoldDatasetFolder(SplitDataset):
    ...
    Methods:
        get_train_dataset(): Return the train dataset
        get_val_dataset(): Return the val dataset
    ...
    def set_holdout(self, holdout=None):
        ...
        if holdout is None:
            self.holdout = holdout
            self._indices['train'] = list(range(len(self.samples)))
            self._indices['val'] = []
        else:
            ...
            self.holdout = holdout
            self._indices['train'] = list(np.where(train_mask)[0])
            self._indices['val'] = list(np.where(~train_mask)[0])
